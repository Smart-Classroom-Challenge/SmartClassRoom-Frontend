...
const IotDevices = await fetch(`http://localhost:8000/api/Classrooms/`, {headers: {'Content-Type': 'application/json', 'Authorization': `Bearer ${store.set_jwt}`}}).then((r) => r.json()).then((data) => data.results);
...
<div class="container px-4 mx-auto my-12 md:px-12">
    <div class="flex flex-wrap -mx-1 lg:-mx-4">
        ...
        <!-- Column -->
        <div v-for="file in IotDevices" :key="file.source" class="w-full px-1 my-5 md:w-1/2 lg:my-4 lg:px-4 lg:w-1/3">

            <!-- Article -->
            <article @click="goToMessurmentStatitons(file.id)" class="overflow-hidden rounded-lg shadow-lg">
